...
app.get('/baskets', async function( req, res ) {
  try {
    ...
    const response = await query(`
      ...
      SELECT DISTINCT ?plu ?aantalPakjes ?totalPrice ?pStuks AS ?besteldStuks ?pGrm AS ?besteldGram ?basket (CONCAT(?firstName, " ", ?lastName) AS ?name) ?address ?phone ?email ?streetAddress ?postalCode ?locality ?user ?lastChange ?hasCustomDeliveryPlace ?deliveryType ?deliveryPlace
      WHERE {
        GRAPH ?g {
          ...
          ?orderLine veeakker:hasOffering ?offering.
          {
            ...
            FILTER NOT EXISTS { ?basket veeakker:invoiceAddress ?address. }
            OPTIONAL {
              ?user
